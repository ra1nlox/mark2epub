...
def get_coverpage_XML(cover_image_path):
    ...
    all_xhtml += """<head>\n</head>\n<body>\n"""
    all_xhtml += (
        """<img src="images/{}" style="height:100%;max-width:100%;"/>\n""".format(
            cover_image_path
        )
    )
    all_xhtml += """</body>\n</html>"""
...
def get_chapter_XML(md_filename, css_filenames):
    ...
    soup = BeautifulSoup(html_text, features="lxml").body

    for tag in soup.contents:
        classes = parse_classes(tag)
        if classes:
            formated_str_of_classes = re.sub(r"\s*{", "", classes[0])
...
if __name__ == "__main__":
    ...
    with zipfile.ZipFile(output_path, "w") as myZipFile:
        ...
        myZipFile.writestr(
            "META-INF/container.xml", container_data, zipfile.ZIP_DEFLATED
        )

        ## Then, the package.opf file itself
        package_data = get_packageOPF_XML(
        ...
        myZipFile.writestr(
            "OPS/TOC.xhtml", toc_xml_data.encode("utf-8"), zipfile.ZIP_DEFLATED
        )

        ## Writing the TOC.ncx file
        toc_ncx_data = get_TOCNCX_XML(all_md_filenames)
        myZipFile.writestr(
            "OPS/toc.ncx", toc_ncx_data.encode("utf-8"), zipfile.ZIP_DEFLATED
        )
        ...
        for i, image_filename in enumerate(all_image_filenames):
            with open(os.path.join(images_dir, image_filename), "rb") as f:
                filedata = f.read()
            myZipFile.writestr(
                "OPS/images/{}".format(image_filename), filedata, zipfile.ZIP_DEFLATED
            )
